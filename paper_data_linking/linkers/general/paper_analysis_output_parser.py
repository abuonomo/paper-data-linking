"""Parser for paper_analysis prompt output.

Tightly coupled to the prompt template at:
    prompts/paper_analysis/system.xml

Changes to the prompt's markdown format (heading levels, field labels,
quote format) must be mirrored here. See _match_field_label() for the
field name contract.
"""
...
def parse_instrument_markdown(text: str) -> StructuredInstrumentDetails:
    ...
    for line in lines:
        ...
        # --- H4 OR bullet-form: New data collection period ---
        # Standard format:  `#### Data Collection Period N: name`
        # Bullet variant:   `- **Data Collection Period N: name**`  (some bedrock papers)
        if stripped.startswith("#### ") or _is_bullet_period_header(stripped):
...
_BULLET_PERIOD_RE = re.compile(
    r"^\s*-\s+\**Data Collection Period\s+\d+:\s*(.*?)\**\s*$"
)


def _is_bullet_period_header(line: str) -> bool:
    """True if `line` is a bullet-form period header.

    Some LLMs (notably bedrock-120b on multi-instrument papers) emit period
    headers as bullet items rather than h4 headings, e.g.:
        - **Data Collection Period 1: First event**
    The standard format is `#### Data Collection Period 1: First event`.
    """
    return bool(_BULLET_PERIOD_RE.match(line))


def _extract_period_name(heading: str) -> str:
    """Extract period name from a period header line.

    Accepts either:
      - H4 heading:    ``#### Data Collection Period 1: Name``
      - Bullet form:   ``- **Data Collection Period 1: Name**``
    """
    # Try the bullet form first; it's strict about its boundaries.
    m = _BULLET_PERIOD_RE.match(heading)
    if m:
        return m.group(1).strip()
    # Fall back to h4 form: strip the leading hashes, then match the label.
...
def _extract_italic_quote(line: str) -> Optional[str]:
    """Extract a bare italic-wrapped quote like *"some text"*.

    Only matches lines that are purely an italic expression — not field labels,
    not sub-bullets, not headings.
    """
    # Full italic-quoted string: *"..."*
...
def _extract_quote(line: str) -> Optional[str]:
    ...
    Handles four format variations the LLM may emit:

      1. Quoted on same line:        ``- **Supporting Quote**: "text"``
      2. Curly-quoted same line:     ``- **Supporting Quote**: \u201ctext\u201d``
      3. Open-quote-only (multi-line continuation):
                                     ``- **Supporting Quote**: "text continues...``
      4. Bare unquoted text:         ``- **Supporting Quote**: text``  (bedrock-format)

    Indented sub-bullet variants of any of the above are also accepted (the
    parser feeds in lines verbatim, including leading whitespace).
    """
    # Quoted on same line (closes with " or \u201d)
    m = re.match(r'^\s*-?\s*\**Supporting Quote\**:\s*["\u201c](.*?)["\u201d]\s*$', line)
    if m:
        return m.group(1).strip()
    # Open-quote without closing on same line (multi-line continuation)
    m = re.match(r'^\s*-?\s*\**Supporting Quote\**:\s*["\u201c](.*)', line)
    if m:
        return m.group(1).rstrip('"\u201d').strip()
    # Bare unquoted text after the label (bedrock-format)
    m = re.match(r'^\s*-?\s*\**Supporting Quote\**:\s*(\S.*?)\s*$', line)
    if m:
        return m.group(1).strip()
    return None
...
def _build_period(period_data: dict) -> dict:
    ...
    # If time_range is still empty after parsing (italic quote or other format
    # not captured inline), fall back to the first time_quote.
