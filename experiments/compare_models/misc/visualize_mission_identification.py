"""
Visualize mission identification self-consistency results across 5 runs.
"""
import json
...
def calculate_fleiss_kappa(responses: List[List[Any]]) -> float:
    ...
    # Get all unique categories (convert all to strings, handle None)
    all_categories = set()
...
def main():
    ...
    print(f"Loading {len(runs_data)} runs...")
    ...
    unknown_involved = sum(1 for responses in responses_by_case
                          if len(set(responses)) > 1 and 'UNKNOWN' in responses)
    different_indices = sum(1 for responses in responses_by_case
                           if len(set(responses)) > 1 and 'UNKNOWN' not in responses)

    print(f"\nDisagreement Types:")
    print(f"  UNKNOWN vs specific: {unknown_involved} cases")
    ...
    values = [perfect_agreement, unknown_involved, different_indices]
    colors_agree = ['#4CAF50', '#FFC107', '#FF9800']
    ...
    # Save figure
    output_path = Path("experiments/compare_models/prompt_experiments/bedrock_120b_mission_identification_run1") / "mission_identification_analysis.png"
