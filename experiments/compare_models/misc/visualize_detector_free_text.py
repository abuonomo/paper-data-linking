"""
Visualize detector_free_text self-consistency results across 5 runs.
"""
import json
...
def calculate_fleiss_kappa(responses: List[List[str]]) -> float:
    ...
    Args:
        responses: List of [response_list] where each response_list contains
                  responses from all runs for one case
    ...
    for case_responses in responses:
        all_categories.update(case_responses)
...
def main():
    experiment_dir = Path("experiments/compare_models/prompt_experiments/detector_free_text_bedrock_120b_full")

    # Find all JSONL files
    jsonl_files = sorted(experiment_dir.glob("bedrock_openai.gpt-oss-120b-1_0_*.jsonl"))

    if len(jsonl_files) != 5:
        print(f"Error: Expected 5 runs, found {len(jsonl_files)}")
    ...
    for case_idx in range(n_cases):
        ...
        for run in runs_data:
            ...
            if parsed:
                detector = parsed.get('detector', 'PARSE_ERROR')
            else:
                detector = 'PARSE_ERROR'
            case_responses.append(detector)
    ...
    print(f"\nAgreement Analysis:")
    print(f"  Perfect agreement (5/5): {perfect_agreement}/{n_cases} ({100*perfect_agreement/n_cases:.1f}%)")
    ...
    for start, end, label, color in regions:
        if start >= -0.2:  # Only show from -0.2 onwards
    ...
    ax1.set_title('Self-Consistency Score\n(5 runs, 100 cases)', fontsize=12, fontweight='bold')
    ax1.set_yticks([])
    ...
    # Panel 3: Top detector responses
    ax3 = axes[2]
    ...
    ax3.set_xlabel('Frequency (across all 5 runs)', fontsize=12, fontweight='bold')
    ax3.set_title('Top 15 Detector Responses', fontsize=12, fontweight='bold')
    ax3.invert_yaxis()
    ...
    # Save figure
    output_path = experiment_dir / "detector_free_text_analysis.png"
